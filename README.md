...
Eval code lives in `backend/eval/`. The pre-2026-09-21 data files
(`gold.jsonl`, `chunks.jsonl`, `contexts.json`, `results.jsonl`, and
others) described a corpus state that no longer existed and were moved to
`backend/eval/_archive_stale/` — see the README there.

**As of commit `94e6e42` (2026-09-21/22), current numbers exist** — a
fresh, deterministic chunk set, gold labels checked against it
(`eval/check_gold_integrity.py` passes), and a real retrieval + generation
eval run. Full detail and exact commit citations are in `TODO.md`'s "Real
evaluation results" section; summary:

- **Retrieval** (n=15 answerable, dense/hybrid/bm25+dense): recall@5
  0.93–1.00, MRR 0.75–0.85, nDCG@5 0.82–0.86 across configs. n=15 is thin;
  do not treat small deltas between configs as significant.
- **Generation** (`openai/gpt-oss-20b`, n=18): avg chrF 0.413 (0.241 above
  the 0.172 unrelated-pair floor), avg embedding-sim 0.729 (0.205 above
  the 0.524 floor).
- **A live hallucination was reproduced and is documented in TODO.md**:
  a query about a pest not in the corpus caused the model to present a
  different pest's pesticide dosage as the answer, passing every current
  safety layer. This is the single most important finding from this
  eval pass — see TODO.md item #43.
- The threshold sweep ran, but its naive best-F1 recommendation
  (TNR=0 on both subsets) was rejected as unsafe to adopt — see TODO.md
  for why. Current uncalibrated thresholds were deliberately left
  unchanged.
- Gold labels were AI-generated by reading actual chunk content against
  each question, at the project owner's explicit direction — not
  independent human verification. See TODO.md's "Still blocked" section.

Do not carry any number on this page forward past its cited commit
without re-running the pipeline — see `ARCHITECTURE.md` Section 41.
