...
| 10 | ~~BM25 evaluation, bucketed by query type~~ — ✅ **RUN** 2026-09-21/22 (bm25+dense config only, per prior decision -- see "Real evaluation results"). bm25-alone and bm25+hybrid still not run; add if a fuller ablation is wanted | 1 |
...
| 43 | **Entity-match hook (ARCHITECTURE.md Section 21, Layer 3) — prototyped 2026-09-23, NOT deployed.** A lexical/rank-based version (`backend/services/entity_match.py`) was built and tested against all 18 real questions' actual retrieved contexts. It correctly catches the motivating case (pest-5), but every rank-cutoff variant tried produces real false positives -- at best, 4 of 15 genuinely answerable questions would be incorrectly refused (disease-3, pest-2, fertilizer-3, general-2), because their retrieval rank-1 hit wasn't even the gold chunk yet generation still succeeded from a lower-ranked correct chunk. A 27-47% false-refusal rate is not a net safety win. Needs either real NER/LLM-based entity extraction, or fixing retrieval quality for named-entity queries first (rank-1 missing an exact lexical match like disease-3's is itself suspicious -- possibly a BM25-favors-this-case situation, check against #10 once more gold data exists). See the module docstring for full test results before re-attempting | 1 |
| 44 | Faithfulness judge prompt doesn't handle refusals — scored two objectively-correct refusals as 0.0 faithfulness in this session's eval run (not a bug fix, needs prompt-calibration work: either skip judging when the generated answer IS a refusal, or give the judge explicit instruction that declining to answer is faithful when the context doesn't cover the question) | 1 |
