...
4. **Human gold-labeling** — **partially unblocked this session, with an important
   ...
   commit's message. **Re-verified with an independent second AI pass 2026-09-24** (see
   dated section above) -- no discrepancies found, integrity check passes. This is still
   NOT independent human domain-expert verification, and a second AI pass cannot become
   one no matter how carefully it's done -- it can catch a labeling/typo error but not a
   case where the corpus itself states a wrong agricultural fact. Before these labels go in
