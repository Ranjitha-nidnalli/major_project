...
| 46 | **`pest-2` refused by an unrelated, uncalibrated threshold — found 2026-09-23 during #43's end-to-end verification.** A genuinely answerable, safety-critical question (Termite control) gets refused not by entity-match but by the Layer 1/2 relevance gate: its dense-only relevance score (0.497) sits just under `UNCALIBRATED_SAFETY_CRITICAL_THRESHOLD` (0.50). entity_match never runs for this case since relevance fails first. A concrete, real instance of the "do not treat these thresholds as calibrated" risk already documented -- not fixed here, needs Phase 2's real threshold recalibration with more gold data (see #7) | 1 |
