...
| 50 | ~~**The LLM router was silently failing, not misclassifying**~~ — ✅ **FIXED 2026-09-25** (branch `fix/kannada-unit-boundaries`, which carries this commit too). Behind #47: the "router labels 7 of 8 pest/disease questions general" finding was wrong in cause. The router call had `max_tokens=100`; replaying it on all 18 questions, 16/18 hit `finish_reason=length` during hidden reasoning (same model behaviour as #48) and 14 returned empty content, which the pipeline silently mapped to `general` ("Router failed" appears 14 times in each of the three 2026-09-25 live logs). With a 1024-token budget: 18/18 finish, 15/18 correct vs the gold category (misses: pest-3→disease, general-2→fertilizer, general-3→pest/disease), stable across two runs. Fix: `services/router_parse.parse_router_category` accepts only a valid category; the router tries 1024 tokens, retries once at 4096, then falls back to `general` with an explicit log (safe, because #47 still applies the protections from the retrieved chunks). The prompt now asks for the category only: the English keywords it also asked for were never read after the web fallback was removed, were often wrong (ಗೆದ್ದಲು/termites → "weevil", black beetle → "black stem borer"), and dropping them halved reasoning tokens (max 539 → 350) with the same 15/18 accuracy. Zero-cost gate harness with the router's measured labels: 12/18 gate decisions correct, vs 13/18 with every question routed general. The difference is general-3 (seed-sett treatment, expected answer a 0.1% Carbendazim fungicide dip): routed `disease`, then refused by entity_mismatch because the query names no disease. Kept, since refusal is the safe direction and the label is defensible. It also restores the strict 0.50 threshold for fertilizer questions, which the silent fallback had been lowering to the 0.35 general threshold. About 72 Groq calls used on diagnosis (18 were an accidental repeat of the old prompt, which confirmed its results are stable). **Live run attempted 2026-09-25 23:00 — INVALID, do not report its numbers.** Groq's free-tier daily token limit (200,000 tokens/day for `openai/gpt-oss-20b`) ran out at fertilizer-1; every later call returned HTTP 429, so the run file was moved to `backend/eval/_archive_stale/invalid_runs/`. The valid first 8 questions: router answered all 8, labels matched the zero-cost measurement (pest-3→disease the only miss), gate decisions matched the harness. After the limit hit, every failed router/judge call ended in a refusal, so fail-closed held. **Also learned:** the 429 messages show "Requested 1711" for a 1024-token call, i.e. Groq's limiter counts the requested `max_tokens` up front, not tokens used. The 1024/4096 budgets (#48, #50) therefore use daily quota faster than their real usage, and 200k tokens/day covers only about 2–3 full live runs. Needs a real run once the quota resets. **Budgets lowered 2026-09-25** (branch `fix/token-budgets`), sized from measured use: judge (1024, 2048), since it used 99-624 tokens over 36 calls and varies run to run; router (512, 2048), since it used 54-361 over 18 calls. That cuts the worst-case requested tokens per question from 8,192 (4096 retries for router and judge) to 4,096, and the router's first try from 1024 to 512. Also checked answer generation, which runs on the same reasoning model with only 250 tokens: every answer in the 22:23 run ends cleanly and nothing was cut off, so it is left alone, but it has the same risk and should be watched (a cut-off dose would still face the numeric check). The paid tier is still needed before any real traffic | 1 |
