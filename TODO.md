...
| 43 | **Build the entity-match hook (ARCHITECTURE.md Section 21, Layer 3) — promoted to top priority 2026-09-22.** Was "highest-value addition, not yet built"; now has a concrete reproduced failure case (see "Real evaluation results": black beetle query hallucinated the Termite dosage, passing every other current safety layer). Extract named entities from the query and retrieved cards; refuse if the query's entity doesn't appear in any retrieved card, regardless of relevance score | 1 |
