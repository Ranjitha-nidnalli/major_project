...
Drafts a candidate gold.jsonl after chunking changes, for a human to review.
...
This is the exact circularity CLAUDE.md's known defect #5 and
ARCHITECTURE.md Section 31 describe: labels generated by retrieving from
the same collection the retriever-under-test also queries. That's why this
writes to gold.jsonl.draft, NOT gold.jsonl -- every other eval script
(run_retrieval_ablation.py, threshold_sweep.py, refusal_test.py) reads
gold.jsonl and is deliberately left pointing at the archived, non-existent
file until a real gold set exists (see backend/eval/_archive_stale/README.md).
Writing straight to gold.jsonl here would silently defeat that "fail loudly
until real data exists" safeguard the moment this script is run.

A human must review gold.jsonl.draft and explicitly rename/copy it to
gold.jsonl -- that rename IS the human verification step CLAUDE.md requires
before this can be treated as real ground truth. Do not automate that step.
...
Output: eval/gold.jsonl.draft (never overwrites gold.jsonl directly)
"""
import os
import sys
import json
...
# Deliberately NOT gold.jsonl -- see module docstring. A human must review
# and explicitly promote this draft before it becomes real ground truth.
GOLD_OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "gold.jsonl.draft")
...
def main():
    ...
    print(f"\n✅ Wrote {len(gold_records)} DRAFT records to {GOLD_OUTPUT_PATH}")
    ...
    print(f"\n⚠️  This is a DRAFT, not real ground truth (CLAUDE.md known defect #5:")
    print(f"   labels generated by the retriever under test are circular).")
    print(f"   1. Review every question in {GOLD_OUTPUT_PATH} by hand against the")
    print(f"      actual source material -- not just spot-check a few.")
    print(f"   2. Fix any chunk that looks wrong.")
    print(f"   3. Only then copy/rename it to gold.jsonl -- eval scripts will keep")
    print(f"      failing with FileNotFoundError on gold.jsonl until you do, by design.")
